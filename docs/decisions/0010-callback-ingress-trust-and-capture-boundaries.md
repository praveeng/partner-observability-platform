...
ADR 0011 fixes the preceding transport boundary: external callbacks arrive through a service-owned 443-only ALB/ACM HTTPS listener to private ECS targets. Port 80 is absent. ALB TLS and forwarded-protocol headers do not establish partner identity; handshake failures before host authentication remain internal-only. The starter never configures listener/application TLS or reads certificate material.
...
This ADR refines ADR 0003 and the ingress portions of ADR 0004. It does not authorize partner authentication from telemetry input. Normative details are in `../architecture.md`, `../security-invariants.md`, `../telemetry-contract.md`, `../payload-policy.md`, `../transport-security.md`, and ADR 0011.
