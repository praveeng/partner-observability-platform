...
ASSESSMENT -> HUMAN APPROVAL -> IMPLEMENTATION. This prompt is the canonical sequence position for the former “Prompt 13B” infrastructure work. Assessment and explicit approval are mandatory before any Terraform modification.
...
This is platform infrastructure work, not broad partner-service discovery. Do not enumerate unrelated `sure-nbfc-*` services. If an exact `TARGET_PARTNER_SERVICE` is supplied for non-secret service-origin requirements, use only that target. Inspect another service only when an infrastructure topology reference makes a narrowly identified shared pattern necessary; limit that read to the referenced infrastructure-facing configuration and never treat it as another integration target. Otherwise derive shared topology from `sure-partner-observability` contracts, SureWebServices shared deployment configuration, and the centralized Terraform repository.
