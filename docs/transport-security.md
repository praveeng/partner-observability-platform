...
Every production-like outbound partner endpoint is a configuration-owned `https` URI with an approved host. Onboarding and CI validation reject an `http` scheme, user-info, an IP literal where hostname verification policy requires DNS, an unapproved port, or a redirect policy that permits an HTTPS-to-HTTP transition. Raw endpoint URLs and query strings never become telemetry.
