...
variable "prom_label_proxy_image" {
  type = string
  validation {
    condition     = can(regex("@sha256:[0-9a-f]{64}$", var.prom_label_proxy_image))
    error_message = "prom-label-proxy image must be pinned by digest."
  }
}
variable "journey_resolver_image" {
  type = string
  validation {
    condition     = can(regex("@sha256:[0-9a-f]{64}$", var.journey_resolver_image))
    error_message = "Journey resolver image must be pinned by digest."
  }
}
...
variable "prom_label_proxy_port" {
  type    = number
  default = 8081
}
variable "journey_resolver_port" {
  type    = number
  default = 8082
}
