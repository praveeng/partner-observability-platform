...
- Verification on 2026-08-17: `scripts/build.sh`, `scripts/test.sh`, and `scripts/test-security.sh --core` pass. `scripts/test-security.sh`, `scripts/test-performance.sh`, and therefore `scripts/verify-all.sh` remain non-zero because Alloy/Loki/Grafana end-to-end security and all six exact M9 performance profiles are explicitly `NOT IMPLEMENTED`; no whole-platform green or performance claim is made.
...
The schema-2 M2/M3 SDK slice is ready for review. M3 now carries bounded TLS metadata and proves no-TLS-mutation behavior plus trusted/untrusted/wrong-host equivalence for all three supported outbound clients. The remaining certificate/redirect matrix, ALB/ACM/private-target deployment policy, callback ingress, rotation, and end-to-end sink evidence stays in M8/M9. M4 remains open for explicit pre-encryption/post-decryption capture and Alloy schema-2 defense in depth; M5-M8 backend/isolation/deployment work and exact M9 duration/throughput profiles are intentionally `NOT IMPLEMENTED`. Aggregate verification must remain non-zero until those downstream gates exist.
