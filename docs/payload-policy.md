...
Selected SLF4J compatibility never treats a rendered line as sanitizable input. A startup manifest must match the exact unformatted template and logger/package pattern, may require an exact marker, and defines the only scalar argument indexes, names, types, and field policies eligible for projection. The template, unselected arguments, MDC, throwable proxy, exception message, and stack trace are not copied or queued. Projected arguments pass through this policy unchanged: credentials/Authorization/card/OTP values are removed, mask-required values are masked, binary/Base64 and oversized candidates are omitted, and unsupported objects fail closed without `toString()`.

An omitted unsafe argument may leave only the configured category/journey/outcome metadata. That category is configuration-owned and bounded; it is not derived from the log message. Missing trusted partner context omits the entire partner record.
